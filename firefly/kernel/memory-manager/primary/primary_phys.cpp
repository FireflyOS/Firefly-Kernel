...
// // std::vector<BuddyTree*> trees_ordering;      // this would be 'zones' but sorted by size (smallest->largest)
...
// static int top_index_in_tree_ordering = 0;
...
//     for (const auto &x : trees_ordering)
//         info_logger << "zone#" << x->zone_id << ", idx: " << x->index_in_tree_ordering << '\n';

//     top_index_in_tree_ordering = trees_ordering.front()->index_in_tree_ordering;
...
//     // auto index = log2(size) + top_index_in_tree_ordering - 12;  // log2(4096) is 12, so we use 12 as the base offset in order to generated zero indexed values for the vector
//     // auto zone = trees_ordering[index];

//     // info_logger << "Good zone for size " << size << " (index: " << index << ") is: " << zone->zone_id << ", " << info_logger.hex(zone->base) << " - " << info_logger.hex(zone->base + zone->length - 1) << logger::endl;
//     // info_logger << "(smallest: " << info_logger.hex(trees_ordering[tree_ordering_largest - 1]->base) << "-" << info_logger.hex(trees_ordering[tree_ordering_largest - 1]->base + trees_ordering[tree_ordering_largest - 1]->length) << ")\n";

//     // auto ptr = zone->allocator.alloc(size, fill);

//     // if (!ptr) {
//     // info_logger << "Completely allocated zone#" << active_zone->zone_id << " : " << info_logger.hex(active_zone->base) << '\n';
//     // std::swap(trees_ordering[index], tree_ordering[tree_ordering_largest]);
//     //     if (zones.empty())
//     //         panic("OOM");
