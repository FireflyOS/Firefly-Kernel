...
bool ps2::init() {
    return (firefly::inb(status_register) & status::system_flag) ? true : false;
}
...
firefly::std::optional<unsigned char> ps2::get_scancode() {
    // ensure there is data to be read
    if (firefly::inb(status_register) & status::out_buffer_status)
        return firefly::inb(data_port);
    return nullptr;
}

void ps2::handle_input(unsigned char scancode, cursor& crs) {
