...
void setup_pages(unsigned long kernel_size) {
    ...
    page_map *pml4 = reinterpret_cast<page_map *>(0x0000);
    ...
    pdpt->entries[0] = pdpt->entries[510] = { true,
                                              true,
                                              false,
                                              false,
                                              false,
    ...
    for (unsigned int pd_entry = 0; target < kernel_end && pt_start < 0xf000; pd_entry++, pt_start += 0x1000) {
        ...
        page_table *pt = reinterpret_cast<page_table *>(pt_start);

        for (int pt_entry = 0; pt_entry < 512 && target < kernel_end; pt_entry++, target += 4096) {
            pt[pt_entry] = { true, true, false, false, false, false, false, false, 0, target >> 12, 0 };
        }
    }
